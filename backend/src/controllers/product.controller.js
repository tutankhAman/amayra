...
const createProduct = asyncHandler(async (req, res) => {
    ...
    const { name, productId, description, price, discount, type, category, sizes, stock, tags } = req.body
    ...
    const newProduct = await Product.create({
        ...
        images,
        stock,
        tags: tags || [] // Add tags with default empty array
    })
...
const getProduct = asyncHandler(async (req, res) => {
    try {
        const { category, type, minPrice, maxPrice, sizes, sortBy, tags, page = 1, limit = 20 } = req.query;
...
const updateProduct = asyncHandler(async (req, res) => {
    ...
    if (typeof tagsArray === 'string') {
        tagsArray = tagsArray.split(',').filter(tag => tag.trim());
    }
    ...
    const product = await Product.findByIdAndUpdate(
        productExistance._id,
        {
            $set: {
                ...
                images: imageUrls,
                tags: tagsArray || [] // Add tags with default empty array
            }
        },
        { new: true }
    )
    ...
    try {
        ...
        const searchQuery = {
            $or: [
                ...
                { tags: { $in: [new RegExp(search, 'i')] } } // Add search by tags
            ]
        };
